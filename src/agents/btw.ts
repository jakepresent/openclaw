...
export async function runBtwSideQuestion(
  ...
  // for github-copilot in the btw runtime), pass `streamSimple` as the wrap
  // base so the copilot wrap can still attach IDE headers on top of the
  // default transport. Without this fallback, `wrapCopilotAnthropicStream`
  // short-circuits to undefined and the caller silently falls back to bare
  // `streamSimple`, which has no IDE headers and 400s on the Copilot API.
